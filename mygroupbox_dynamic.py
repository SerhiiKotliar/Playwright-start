...
class MyGroupBox(_BaseGroupBox):
    """
    QGroupBox с сигналами focusEntered и focusLeft.
    - focusEntered испускается, когда фокус переходит в любой дочерний виджет группы.
    - focusLeft испускается, когда фокус уходит за пределы группы.
    """
    ...
    def eventFilter(self, obj, event):
        if event.type() in (QEvent.FocusIn, QEvent.FocusOut):
            ...
            if event.type() == QEvent.FocusIn:
                # если новый фокус внутри группы
                if new_focus is not None and self.isAncestorOf(new_focus):
                    try:
                        self.focusEntered.emit()
                    except Exception:
